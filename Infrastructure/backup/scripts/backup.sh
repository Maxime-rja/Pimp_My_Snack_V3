...
sudo mysqldump --databases pimpmysnack > /vagrant/backup/dump/sauvegarde-$(date +%Y%m%d%H%M%S).sql
...
sudo mysqldump --databases pimpmysnack > /vagrant/backup/dump/latest-save.sql
